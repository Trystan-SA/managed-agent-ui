  ...
  onMount(async () => {
    try {
      ...
      if (pkgs) {
        ...
        cargoPackages = (pkgs.cargo ?? []).join(', ');
        goPackages = (pkgs.go ?? []).join(', ');
        gemPackages = (pkgs.gem ?? []).join(', ');

        // Expand package rows that have content
        for (const pm of packageManagers) {
          if (pm.get().trim()) expandedPkgs.add(pm.key);
        }
      }
    } catch (_e: unknown) {
...
<div class="edit-env">
  <div class="edit-env__header">
    ...
    <h1 class="edit-env__title">Edit Environment</h1>
    <p class="edit-env__subtitle">Update the environment configuration</p>
  </div>
  ...
  {:else}
    ...
    <form class="env-form" onsubmit={(e) => { e.preventDefault(); handleSubmit(); }}>
      ...
      <div class="form-footer">
        <a href="/environments/{envId}" class="btn--cancel">Cancel</a>
